    ...
    if (recordId) {
      ...
      fetch(`${API_URL}/patientRecords/${recordId}`, {
        ...
        .then((record) => {
          ...
          document.getElementById("priorConditions").value =
            record.medicalHistory.priorConditions.join(", ");
          document.getElementById("allergies").value =
            record.medicalHistory.allergies.join(", ");
          document.getElementById("currentMedications").value =
            record.medicalHistory.currentMedications.join(", ");
        })
        .catch((error) => {
          recordError();
        })
...
function saveChanges(recordId) {
  ...
  const priorConditions = document
    .getElementById("priorConditions")
    .value.split(",")
    .map((item) => item.trim());
  ...
  const currentMedications = document
    .getElementById("currentMedications")
    .value.split(",")
    .map((item) => item.trim());
  ...
  const url = recordId
    ? `${API_URL}/patientRecords/${recordId}`
    : `${API_URL}/patientRecords`;
